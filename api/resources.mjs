...
function resource(app, route, table, dateColumn, fields) {
  ...
  app.delete(`/api/${route}/:id`, asyncRoute(async (req, res) => { const row = await one(db, `delete from ${table} where id=$1 returning *`, [req.params.id], 'Data tidak ditemukan.'); await audit(db, route, row, 'delete'); res.json({ ok: true }) }))
}
